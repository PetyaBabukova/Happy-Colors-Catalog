...
- `coverage.all: true` се задава, за да включи и untouched файлове в denominator-а; иначе threshold-ът оценява само touched code и не отразява реалното покритие
...
- `happy-colors-nextjs-project/`: unit + unit-jsdom + components + api combined coverage
...
Този coverage блок се поставя в същия `test` object, който съдържа `projects` array-а в реалните `vitest.config.js` файлове. Frontend config-ът използва `include: ['src/**/*.{js,jsx}']`; server config-ът използва equivalent include като `['**/*.js']` със същите excludes, така че да не включва `node_modules`, config файлове или `dev-server.js`.
...
- frontend има unit, unit-jsdom и component Vitest projects
