...
async function loadRoute() {
  return import('../../../src/app/api/revalidate/cartoon-hero-banners/route.js');
}

describe('/api/revalidate/cartoon-hero-banners', () => {
  beforeEach(() => {
  ...
  it('revalidates the cartoon banner tag and cartoons path', async () => {
  ...
  it('allows internal revalidation requests with the configured secret', async () => {
    vi.stubEnv('CARTOON_HERO_BANNER_REVALIDATE_SECRET', 'server-secret');
    authResult = { ok: false, status: 401, message: 'Missing authentication token.' };
    const { POST } = await loadRoute();
